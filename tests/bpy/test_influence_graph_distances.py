...
def test_identical_signed_graphs_have_full_jaccard_similarity():
    ...
    assert bt.bpy.ig.similarity(graph, graph) == 1.0
    assert bt.bpy.ig.distance(graph, graph) == 0.0
...
def test_disjoint_signed_graphs_have_zero_jaccard_similarity():
    ...
    assert bt.bpy.ig.similarity(graph1, graph2) == 0.0
    assert bt.bpy.ig.distance(graph1, graph2) == 1.0
...
def test_signed_jaccard_similarity_counts_partial_overlap():
    ...
    assert bt.bpy.ig.similarity(graph1, graph2) == pytest.approx(1 / 3)
    assert bt.bpy.ig.distance(graph1, graph2) == pytest.approx(2 / 3)
...
def test_signed_hamming_uses_complete_node_union_domain_by_default():
    ...
    assert bt.bpy.ig.similarity(graph1, graph2, metric="hamming") == pytest.approx(
        15 / 16
    )
    assert bt.bpy.ig.distance(graph1, graph2, metric="hamming") == pytest.approx(1 / 16)
...
def test_signed_hamming_uses_explicit_domain():
    ...
    assert bt.bpy.ig.similarity(
        graph1,
        graph2,
        metric="hamming",
        domain=dorothea_like_domain,
    ) == pytest.approx(1 / 2)
    assert bt.bpy.ig.distance(
        graph1,
        graph2,
        metric="hamming",
        domain=dorothea_like_domain,
...
def test_signed_jaccard_similarity_distinguishes_opposite_signs():
    ...
    assert bt.bpy.ig.similarity(positive, negative) == 0.0
    assert bt.bpy.ig.distance(positive, negative) == 1.0
...
def test_signed_jaccard_similarity_ignores_edge_attributes():
    ...
    graph1 = bt.bpy.ig.InfluenceGraph()
    graph1.add_edge("A", "B", sign=1, evidence="curated")

    graph2 = bt.bpy.ig.InfluenceGraph()
    graph2.add_edge("A", "B", sign=1, confidence=0.7)

    assert bt.bpy.ig.similarity(graph1, graph2) == 1.0
    assert bt.bpy.ig.distance(graph1, graph2) == 0.0
...
def test_empty_graphs_have_full_jaccard_similarity():
    ...
    assert bt.bpy.ig.similarity(graph1, graph2) == 1.0
    assert bt.bpy.ig.distance(graph1, graph2) == 0.0
    assert bt.bpy.ig.similarity(graph1, graph2, metric="hamming") == 1.0
    assert bt.bpy.ig.distance(graph1, graph2, metric="hamming") == 0.0
...
def test_unsupported_similarity_and_distance_metrics_raise_clear_errors():
    ...
    with pytest.raises(ValueError, match="unsupported similarity metric"):
        bt.bpy.ig.similarity(graph1, graph2, metric="cosine")

    with pytest.raises(ValueError, match="unsupported distance metric"):
        bt.bpy.ig.distance(graph1, graph2, metric="cosine")
