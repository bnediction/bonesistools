...
from bonesistools.omics.tools import _utils
...
def test_get_expression_selects_x_layer_and_raw(mini_adata):
    ...
    expression_mtx_copy = cast(np.ndarray, bt.omics.tl.get_expression(mini_adata))

    assert np.array_equal(expression_mtx_copy, expression_mtx)
    assert expression_mtx_copy is not expression_mtx
    assert np.array_equal(
        cast(np.ndarray, bt.omics.tl.get_expression(mini_adata, copy=False)),
        expression_mtx,
    )
    assert np.array_equal(
        cast(
            np.ndarray,
            bt.omics.tl.get_expression(
                mini_adata,
                layer="counts",
                copy=False,
            ),
...
def test_get_representation_truncates_dimensions(mini_adata):
    ...
    full_representation_mtx = bt.omics.tl.get_representation(
        mini_adata,
        obsm=None,
    )
    representation_mtx = cast(
        np.ndarray,
        bt.omics.tl.get_representation(
            mini_adata,
            obsm="X_pca",
            n_components=2,
        ),
...
def test_get_representation_reports_missing_key(mini_adata):
    with pytest.raises(KeyError, match="key 'missing' not found in scdata.obsm"):
        bt.omics.tl.get_representation(mini_adata, obsm="missing")

    pca_missing = mini_adata.copy()
    del pca_missing.obsm["X_pca"]

    with pytest.raises(KeyError, match="bonesistools.omics.tl.pca"):
        bt.omics.tl.get_representation(pca_missing, obsm=None)
...
def test_get_representation_deprecates_use_rep(mini_adata):
    ...
    with pytest.warns(FutureWarning, match="`use_rep` is deprecated"):
        use_rep_mtx = bt.omics.tl.get_representation(mini_adata, use_rep="X_pca")
...
def test_get_pairwise_selects_obsp_and_varp(mini_adata):
    ...
    obs_pairwise_mtx = bt.omics.tl.get_pairwise(mini_adata, "connectivities")
    var_pairwise_mtx = bt.omics.tl.get_pairwise(
        mini_adata,
        key="correlations",
        axis="var",
    )
