...
def test_to_dataframe_handles_sparse_x_sparse_layer_and_obs_string(mini_adata):
    ...
    x_df = bt.omics.tl.to_dataframe(sparse_adata)
    ...
    layer_df = bt.omics.tl.to_dataframe(
        sparse_adata,
        obs="cluster",
...
def test_deprecated_anndata_to_dataframe_warns_and_delegates(mini_adata):
    with pytest.warns(FutureWarning, match="bt.omics.tl.to_dataframe"):
        deprecated = getattr(bt.omics.tl, "anndata_to_dataframe")(mini_adata)

    current = bt.omics.tl.to_dataframe(mini_adata)
