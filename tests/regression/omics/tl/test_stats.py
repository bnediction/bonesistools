...
from bonesistools.omics._stats import _column_mean_variance
from bonesistools.omics._typing import Matrix
from bonesistools.omics.tools import _stats
...
def test_wilcoxon_tests_returns_expected_group_statistics():
    ...
    result = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background="rest",
...
def test_wilcoxon_tests_warns_when_memory_is_smaller_than_one_rank_column():
    ...
    expected = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        correction=None,
...
def test_wilcoxon_tests_accepts_boolean_var_subset_column():
    ...
    from_column = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        var_subset="selected",
    ...
    from_names = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
...
def test_wilcoxon_tests_fixed_background_chunking_preserves_default_results():
    ...
    unchunked = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A", "C"],
        background=["B"],
    ...
    small_memory = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A", "C"],
        background=["B"],
...
def test_wilcoxon_tests_supports_composite_background_groups():
    ...
    composite = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["B", "C"],
    ...
    rest = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background="rest",
...
def test_wilcoxon_tests_distinguishes_rest_keyword_from_rest_group():
    ...
    keyword_result = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background="rest",
    ...
    group_result = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["rest"],
...
def test_wilcoxon_tests_uses_sparse_layer():
    ...
    dense_result = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        expression="X",
    )
    sparse_result = bt.omics.tl.wilcoxon_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        expression="counts",
...
def test_wilcoxon_tests_validates_arguments():
    ...
    with pytest.raises(ValueError, match="invalid argument value for 'groups'"):
        bt.omics.tl.wilcoxon_tests(adata, groupby="cluster", groups=["missing"])

    with pytest.raises(ValueError, match="invalid argument value for 'background'"):
        bt.omics.tl.wilcoxon_tests(adata, groupby="cluster", background=["missing"])

    with pytest.raises(ValueError, match="must be disjoint"):
        bt.omics.tl.wilcoxon_tests(
            adata,
            groupby="cluster",
            groups=["A"],
            background=["A"],
    ...
    with pytest.raises(TypeError, match="unsupported argument type for 'background'"):
        bt.omics.tl.wilcoxon_tests(adata, groupby="cluster", background=cast(Any, "B"))
    ...
    with pytest.raises(ValueError, match="expected at least one background group"):
        bt.omics.tl.wilcoxon_tests(adata, groupby="cluster", background=[])
    ...
    with pytest.raises(TypeError, match="unsupported argument type for 'groups'"):
        bt.omics.tl.wilcoxon_tests(adata, groupby="cluster", groups=cast(Any, 1))
    ...
    with pytest.raises(ValueError, match="invalid argument value for 'max_memory'"):
        bt.omics.tl.wilcoxon_tests(adata, groupby="cluster", max_memory="1XB")
...
def test_welch_tests_returns_expected_fixed_background_statistics():
    ...
    result = bt.omics.tl.welch_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["B"],
...
def test_welch_tests_uses_sparse_layer():
    ...
    dense_result = bt.omics.tl.welch_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["B"],
    ...
    sparse_result = bt.omics.tl.welch_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["B"],
...
def test_welch_tests_accepts_boolean_var_subset_column():
    ...
    from_column = bt.omics.tl.welch_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["B"],
    ...
    from_names = bt.omics.tl.welch_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["B"],
...
def test_welch_tests_chunking_preserves_default_results():
    ...
    unchunked = bt.omics.tl.welch_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["B"],
    ...
    chunked = bt.omics.tl.welch_tests(
        adata,
        groupby="cluster",
        groups=["A"],
        background=["B"],
...
def test_welch_tests_validates_arguments():
    ...
    with pytest.raises(ValueError, match="invalid argument value for 'groups'"):
        bt.omics.tl.welch_tests(adata, groupby="cluster", groups=["missing"])

    with pytest.raises(ValueError, match="invalid argument value for 'background'"):
        bt.omics.tl.welch_tests(adata, groupby="cluster", background=["missing"])

    with pytest.raises(ValueError, match="must be disjoint"):
        bt.omics.tl.welch_tests(
            adata, groupby="cluster", groups=["A"], background=["A"]
        )

    with pytest.raises(TypeError, match="unsupported argument type for 'background'"):
        bt.omics.tl.welch_tests(adata, groupby="cluster", background=cast(Any, "B"))

    with pytest.raises(ValueError, match="invalid argument value for 'max_memory'"):
        bt.omics.tl.welch_tests(adata, groupby="cluster", max_memory="1XB")
    ...
    with pytest.raises(
        ...
        bt.omics.tl.welch_tests(small_group, groupby="cluster", groups=["A"])
