...
from bonesistools.omics.preprocessing import _qc
...
def test_qc_numba_backend_requires_numba(monkeypatch):
    ...
    with pytest.raises(ImportError, match="backend='numba' requires"):
        bt.omics.pp.qc(adata, backend="numba")
...
def test_qc_handles_zero_count_observations():
    ...
    bt.omics.pp.qc(adata, qc_vars=["mito"], percent_top=[1])
...
def test_qc_single_observation_has_undefined_feature_variance():
    ...
    bt.omics.pp.qc(adata, percent_top=None)
...
def test_qc_validates_arguments():
    ...
    with pytest.raises(IndexError, match="Positions outside range of features"):
        bt.omics.pp.qc(adata, percent_top=[5])

    with pytest.raises(TypeError, match="unsupported element type in 'qc_vars'"):
        bt.omics.pp.qc(adata, qc_vars=["mito", cast(Any, 1)], percent_top=None)

    with pytest.raises(TypeError, match="unsupported argument type for 'qc_vars'"):
        bt.omics.pp.qc(adata, qc_vars=cast(Any, 1), percent_top=None)

    with pytest.raises(TypeError, match="unsupported argument type for 'percent_top'"):
        bt.omics.pp.qc(adata, percent_top=cast(Any, "1"))

    with pytest.raises(KeyError):
        bt.omics.pp.qc(adata, qc_vars=["missing"], percent_top=None)

    adata.var["bad"] = ["yes", "no", "yes", "no"]
    with pytest.raises(TypeError, match="unsupported column dtype"):
        bt.omics.pp.qc(adata, qc_vars=["bad"], percent_top=None)

    with pytest.raises(ValueError, match="unsupported backend"):
        bt.omics.pp.qc(adata, backend=cast(Any, "bad"))
