...
def test_log1p_dense_expression_layer_in_place():
    ...
    bt.omics.pp.log1p(adata, expression="normalized")
...
def test_log1p_base_two_and_ten():
    ...
    bt.omics.pp.log1p(adata_base2, base=2)
    bt.omics.pp.log1p(adata_base10, base=10)
...
def test_log1p_copy_returns_new_anndata_and_leaves_original_unchanged():
    ...
    copied = bt.omics.pp.log1p(adata, copy=True)
...
def test_log1p_max_memory_chunking_preserves_dense_results():
    ...
    bt.omics.pp.log1p(unchunked, max_memory=None)
    bt.omics.pp.log1p(chunked, max_memory=16)
...
def test_log1p_rejects_invalid_base_values(mini_adata):
    ...
    with pytest.raises(ValueError):
        bt.omics.pp.log1p(mini_adata, base=0)

    with pytest.raises(ValueError):
        bt.omics.pp.log1p(mini_adata, base=-2)

    with pytest.raises(ValueError):
        bt.omics.pp.log1p(mini_adata, base=1)


def test_log1p_rejects_invalid_expression_key_added_and_memory(mini_adata):

    with pytest.raises(TypeError):
        bt.omics.pp.log1p(mini_adata, expression=cast(Any, object()))

    with pytest.raises(TypeError):
        bt.omics.pp.log1p(mini_adata, key_added=cast(Any, object()))

    with pytest.raises(ValueError):
        bt.omics.pp.log1p(mini_adata, max_memory="1XB")
...
def test_normalize_sparse_integer_x_remains_sparse_float32():
    ...
    bt.omics.pp.normalize(adata, target_sum=10)
...
def test_normalize_expression_overwrites_selected_layer_only():
    ...
    bt.omics.pp.normalize(adata, target_sum=8, expression="counts")
...
def test_normalize_key_added_preserves_x_when_expression_is_none():
    ...
    bt.omics.pp.normalize(adata, target_sum=4, key_added="normalized")
...
def test_normalize_target_sum_sets_nonzero_row_sums():
    ...
    bt.omics.pp.normalize(adata, target_sum=25)
...
def test_normalize_copy_returns_new_anndata_and_leaves_original_unchanged():
    ...
    copied = bt.omics.pp.normalize(adata, target_sum=6, copy=True)
...
def test_normalize_matches_scbolt_reference_helper_for_sparse_layer():
    ...
    bt.omics.pp.normalize(candidate, expression="counts", target_sum=1e4)
...
def test_normalize_rejects_invalid_arguments(mini_adata):
    ...
    with pytest.raises(ValueError):
        bt.omics.pp.normalize(mini_adata, target_sum=0)

    with pytest.raises(TypeError):
        bt.omics.pp.normalize(mini_adata, expression=cast(Any, object()))

    with pytest.raises(TypeError):
        bt.omics.pp.normalize(mini_adata, key_added=cast(Any, object()))
...
def test_scale_dense_expression_layer_in_place():
    ...
    bt.omics.pp.scale(adata, expression="log1p")
...
def test_scale_dense_x_to_new_layer_preserves_source():
    ...
    bt.omics.pp.scale(adata, key_added="scaled")
...
def test_scale_dense_expression_layer_to_new_layer_preserves_source():
    ...
    bt.omics.pp.scale(adata, expression="log1p", key_added="scaled")
...
def test_scale_dense_representation_in_place_does_not_update_var():
    ...
    bt.omics.pp.scale(adata, representation="X_pca")
...
def test_scale_zero_center_false_divides_by_standard_deviation_only():
    ...
    bt.omics.pp.scale(adata, zero_center=False)
...
def test_scale_max_value_clips_scaled_values():
    ...
    bt.omics.pp.scale(adata, max_value=1.0)
...
def test_scale_sparse_zero_center_true_densifies_and_scales():
    ...
    bt.omics.pp.scale(adata, zero_center=True)
...
def test_scale_copy_returns_new_anndata_and_leaves_original_unchanged():
    ...
    copied = bt.omics.pp.scale(adata, copy=True)
...
def test_scale_rejects_invalid_argument_types(mini_adata):
    ...
    with pytest.raises(TypeError):
        bt.omics.pp.scale(mini_adata, expression=cast(Any, object()))

    with pytest.raises(TypeError):
        bt.omics.pp.scale(mini_adata, representation=cast(Any, object()))

    with pytest.raises(TypeError):
        bt.omics.pp.scale(mini_adata, key_added=cast(Any, object()))

    with pytest.raises(TypeError):
        bt.omics.pp.scale(mini_adata, zero_center=cast(Any, "yes"))

    with pytest.raises(ValueError):
        bt.omics.pp.scale(mini_adata, max_value=0)
