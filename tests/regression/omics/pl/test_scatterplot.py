...
from bonesistools.omics.plotting import _scatterplot
...
def test_embedding_discrete_uses_colors_from_uns(mini_adata):
    ...
    np.testing.assert_allclose(
        facecolors[0],
        np.asarray(bt.omics.pl.rgba("red", alpha=1.0), dtype=float),
    )
    np.testing.assert_allclose(
        facecolors[1],
        np.asarray(bt.omics.pl.rgba("blue", alpha=1.0), dtype=float),
    )
    assert [
...
def test_embedding_plot_writes_outfile_and_validates_inputs(mini_adata, tmp_path):
    ...
    with pytest.raises(KeyError):
        bt.omics.pl.embedding(mini_adata, obs="missing", representation="X_pca")

    with pytest.raises(KeyError):
        bt.omics.pl.embedding(mini_adata, obs="cluster", representation="missing")
...
def test_embedding_plot_discrete_3d_reuses_axes_and_customizes_ticks(mini_adata):
    ...
    returned_fig, returned_ax = bt.omics.pl.embedding(
        mini_adata,
...
def test_embedding_plot_continuous_accepts_registered_colormap(mini_adata):
    ...
    fig, ax = bt.omics.pl.embedding(
        ...
        obs="score",
        representation="X_pca",
        colors=bt.omics.pl.get_colormap("earth"),
    )
