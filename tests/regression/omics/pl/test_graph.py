...
from bonesistools.omics.plotting import _graph as graph_plotting
...
def test_paga_graph_reads_nested_connectivities_as_undirected(mini_adata):
    bt.omics.tl.paga(mini_adata, groupby="cluster")
...
def test_graph_overlay_deprecates_label_kwargs(mini_adata):
    ...
    with pytest.warns(FutureWarning, match="`label_kwargs` is deprecated"):
        returned_ax = bt.omics.pl.graph_overlay(
            mini_adata,
            graph_key="epg",
            labels=True,
