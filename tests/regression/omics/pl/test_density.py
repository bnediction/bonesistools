...
from bonesistools.omics.plotting import _density
...
def test_density_with_obs_expression_mapping_ax_outfile_and_errors(
    ...
    returned_fig, returned_ax = bt.omics.pl.density(
        mini_adata,
        feature="g1",
    ...
    assert bt.omics.pl.density(mini_adata, feature="g1", outfile=outfile) is None
    assert outfile.exists()

    with pytest.raises(ValueError, match="invalid argument values"):
        bt.omics.pl.density(mini_adata, feature="g1", show_global=False)

    with pytest.raises(KeyError):
        bt.omics.pl.density(mini_adata, feature="g1", obs="missing")
...
def test_cdf_with_obs_mapping_ax_outfile(mini_adata, tmp_path):
    ...
    returned_fig, returned_ax = bt.omics.pl.cdf(
        mini_adata,
        feature="g1",
        expression="sparse_counts",
        obs="cluster",
    ...
    assert bt.omics.pl.cdf(mini_adata, feature="g1", outfile=outfile) is None
    assert outfile.exists()
...
@pytest.mark.parametrize("function", [bt.omics.pl.density, bt.omics.pl.cdf])
