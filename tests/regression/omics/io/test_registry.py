...
import bonesistools.omics.datasets as datasets_module
from bonesistools.omics.input_output import _pbmc3k as pbmc3k_module
from bonesistools.omics.input_output import _registry as registry_module
...
def test_io_load_pbmc3k_returns_raw_counts(monkeypatch, tmp_path):
    ...
    first = bt.omics.io.load("pbmc3k", quiet=True)
    second = bt.omics.io.load("pbmc3k", quiet=True)
...
def test_io_load_rejects_invalid_cached_shape(monkeypatch, tmp_path):
    ...
    with pytest.raises(
        ...
        bt.omics.io.load("pbmc3k", quiet=True)
...
def test_io_info_nestorowa_contains_source_license_and_counts_url():
    metadata = bt.omics.io.info("nestorowa")
...
def test_io_available_lists_registered_datasets():
    datasets = bt.omics.io.available()
...
def test_dataset_metadata_entries_follow_documented_conventions():
    ...
    for name in datasets.index:
        metadata = bt.omics.io.info(str(name))
...
def test_deprecated_datasets_namespace_hides_technical_imports():
    datasets = getattr(bt.omics, "datasets")
