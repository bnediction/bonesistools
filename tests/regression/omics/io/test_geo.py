...
from bonesistools.omics.input_output import _geo as geo
...
def test_geo_validates_gsm_accessions():
    ...
    with pytest.raises(TypeError):
        bt.omics.io.from_geo(cast(Any, 12345), quiet=True)
    ...
    with pytest.raises(ValueError):
        bt.omics.io.from_geo("GSE12345", quiet=True)

    with pytest.raises(ValueError):
        bt.omics.io.from_geo("foo", quiet=True)
