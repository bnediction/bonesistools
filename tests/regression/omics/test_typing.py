...
from bonesistools.omics import _typing
...
def test_sct_typing_alias_is_not_exposed_publicly():
    assert "typing" not in dir(bt.omics)

    with pytest.raises(AttributeError):
        getattr(bt.omics, "typing")


def test_sct_namespace_exposes_short_aliases_only():
    assert {"pp", "tl", "io", "pl"} <= set(dir(bt.omics))
    for name in [
        "datasets",
        "input_output",
        "plotting",
        ...
        assert name not in dir(bt.omics)
...
def test_mudata_checker_falls_back_when_missing(
    ...
    typing_module = _load_typing_module(
        monkeypatch,
        "bonesistools_omics_typing_without_mudata",
...
def test_typing_module_supports_import_compatibility_fallbacks(monkeypatch):
    literal_module = _load_typing_module(
        monkeypatch,
        "bonesistools_omics_typing_literal_fallback",
        force_typing_literal_fallback=True,
    )
    legacy_module = _load_typing_module(
        monkeypatch,
        "bonesistools_omics_typing_legacy_importlib",
