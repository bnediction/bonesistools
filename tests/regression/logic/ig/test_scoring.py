...
from bonesistools.logic.influence_graph import _scoring
...
def test_influence_graph_scoring_functions_are_public_without_score_constructor():
    ...
    assert "InteractionScore" not in dir(bt.logic.ig)
    assert not hasattr(bt.logic.ig, "InteractionScore")
    assert callable(bt.logic.ig.infer_signed_interactions)
    assert callable(bt.logic.ig.infer_signed_interactions_from_walks)
    assert callable(bt.logic.ig.interaction_scores_from_walks)
...
def test_interaction_scores_from_walks_cancels_opposite_signed_paths():
    ...
    scores = bt.logic.ig.interaction_scores_from_walks(
        graph,
        genes=["A", "D"],
        max_depth=2,
    )
...
def test_scores_from_walks_handles_digraphs_and_missing_sources():
    ...
    with pytest.raises(ValueError, match="expected at least 2 values"):
        bt.logic.ig.interaction_scores_from_walks(
            graph,
            genes=["A", "C"],
            max_depth=2,
            weights=[1.0],
...
def test_infer_signed_interactions_from_walks_matches_two_step_pipeline():
    ...
    expected = bt.logic.ig.infer_signed_interactions(
        scores,
        genes=["A", "D"],
        threshold=0.6,
    )
    ...
    assert (
        bt.logic.ig.infer_signed_interactions_from_walks(
            graph,
            genes=["A", "D"],
            max_depth=2,
            threshold=0.6,
...
def test_infer_signed_interactions_keeps_clear_dominant_direction():
    ...
    assert bt.logic.ig.infer_signed_interactions(
        scores,
        genes=["A", "B"],
        threshold=0.75,
    ) == [("A", "B", {"sign": 1})]
...
def test_infer_signed_interactions_rejects_weak_or_ambiguous_scores():
    ...
    assert (
        bt.logic.ig.infer_signed_interactions(
            scores,
            genes=["A", "B"],
            threshold=0.75,
        )
...
def test_interaction_scoring_rejects_invalid_arguments_and_signs():
    ...
    with pytest.raises(ValueError):
        bt.logic.ig.infer_signed_interactions({}, threshold=2)

    with pytest.raises(ValueError):
        bt.logic.ig.infer_signed_interactions({}, minimum_path_number=0)
