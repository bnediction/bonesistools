...
from bonesistools.logic.boolean_algebra import _typing
...
def test_typing_helpers_are_not_exposed_publicly():
    for name in [
        ...
        assert name not in dir(bt.logic.ba)
        assert not hasattr(bt.logic.ba, name)


def test_bpy_namespace_exposes_short_aliases_only():
    assert {"ba", "bn", "ig", "io"} <= set(dir(bt.logic))
    for name in [
        "boolean_algebra",
        "boolean_network",
        "influence_graph",
        ...
        assert name not in dir(bt.logic)
        assert not hasattr(bt.logic, name)

    assert "pl" not in dir(bt.logic)
    assert not hasattr(bt.logic, "pl")
