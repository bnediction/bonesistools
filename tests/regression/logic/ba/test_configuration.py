...
def test_configuration_set_iterates_internal_configuration_bitsets():
...
def test_configuration_set_equality_is_semantic_not_representational():
    ...
    compact = bt.logic.ba.ConfigurationSet(["A", "B"], [{"B": 0}])

    expanded = bt.logic.ba.ConfigurationSet(["A", "B"])
    expanded.add({"A": 0, "B": 0})
    expanded.add({"A": 1, "B": 0})

    reordered = bt.logic.ba.ConfigurationSet(["B", "A"], [{"B": 0}])
    different = bt.logic.ba.ConfigurationSet(["A", "B"], [{"A": 0}])
    different_components = bt.logic.ba.ConfigurationSet(["A", "C"], [{"A": 0}])
...
def test_configuration_set_rejects_invalid_inputs():
    ...
    with pytest.raises(ValueError, match="duplicated"):
        bt.logic.ba.ConfigurationSet(["A", "A"])

    configurations = bt.logic.ba.ConfigurationSet(["A"])
