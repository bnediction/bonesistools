...
def test_to_csv_writes_dense_matrix(mini_adata, tmp_path):
    ...
    bt.sct.io.to_csv(mini_adata, filename)
...
def test_to_mtx_writes_layer_matrix(mini_adata, tmp_path):
    ...
    bt.sct.io.to_mtx(mini_adata, filename, layer="counts")
...
def test_to_npz_writes_sparse_matrix(mini_adata, tmp_path):
    ...
    bt.sct.io.to_npz(mini_adata, filename, layer="sparse_counts")
...
def test_writers_cover_layer_and_sparse_x_branches(mini_adata, tmp_path):
    ...
    bt.sct.io.to_csv(mini_adata, tmp_path / "dense_layer", layer="dense_counts")

    mini_adata.X = sparse.csr_matrix(mini_adata.X)

    bt.sct.io.to_mtx(mini_adata, tmp_path / "sparse_x_mtx")
    bt.sct.io.to_npz(mini_adata, tmp_path / "sparse_x_npz")
...
def test_deprecated_tl_writers_warn_and_delegate(
    ...
    with pytest.warns(FutureWarning, match=f"bt.sct.io.{name}"):
        getattr(bt.sct.tl, name)(mini_adata, tmp_path / name)
