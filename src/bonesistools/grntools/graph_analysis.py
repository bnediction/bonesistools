...
def info_strongly_connected_components(G):
    sccs = [
        ...
        if len(scc) == 1 and list(scc)[0] in list(nx.nodes_with_selfloops(G))
    ]
    print(
        f"Number of self-looped nodes that are a strongly connected components : {len(sccs)}"
    )
...
def shortest_path_marker_scc(G, marker, scc):
    ...
    last_len_shortest_path = (
        10000000  # high to begin with, need a proper initialization (but flemme)
    )
    for node_scc in scc:
...
def draw_compressed_graph(G, markers, sccs, outputfile):
    ...
    try:
        ...
        print(
            f"Please run the following command in your terminal : dot -Tpdf {outputfile}.dot -o {outputfile}.pdf"
        )
        print("#### END ####")
