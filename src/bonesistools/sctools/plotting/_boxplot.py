    ...
    if hue is None:
        ...
        bps = plt.boxplot(
            x=(
                series.dropna()
                if groupby is None
                else [series.get_group(g).dropna() for g in groups]
            ),
            positions=positions,
            notch=notch,
            sym=sym,
        ...
        if boxitems_to_color:
            for h in hues:
                __apply_box_colors(
                    bp=bps[h], color=box_colors[h], items=boxitems_to_color
                )
        ...
        if showlegend is True:
            ...
            for h in hues:
                handles.append(
                    mpatches.Patch(
                        edgecolor=box_colors[h], facecolor=point_colors[h], label=h
                    )
                )
